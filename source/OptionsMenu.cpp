...
void COptionsMenu::Render( void )
{
	...
	RECT rMusic = {273, 275, 273 + long(213 * m_nMusicVolume / 100.0f), 288};
	...
	RECT rSFX = {273,297,273 + long(213 * m_nSFXVolume / 100.0f),310};
...
bool COptionsMenu::Input( void )	
{
	...
	if(m_fPosY <= 0.0f)
	{
		if(!m_bSubMenu)
		{
			...
			else if(pDI->KeyPressed(DIK_W) || pDI->KeyPressed(DIK_UPARROW) || pDI->JoystickDPadPressed(DIR_UP) || pDI->JoystickGetLStickDirPressed(DIR_UP))
			{
				if(GetCursorSelection() == 0)
		...
		{
			if(pDI->KeyPressed(DIK_ESCAPE) || pDI->KeyPressed(DIK_RETURN) || pDI->JoystickButtonPressed(1) || pDI->JoystickButtonPressed(2))
			{
				m_bSubMenu = false;
				if(CSGD_XAudio2::GetInstance()->SFXIsSoundPlaying(GetSFXID()) == false)
					CSGD_XAudio2::GetInstance()->SFXPlaySound(GetSFXID());
			}
			else if(pDI->KeyPressed(DIK_A) || pDI->KeyPressed(DIK_LEFTARROW) || pDI->JoystickDPadPressed(DIR_LEFT) || pDI->JoystickGetLStickDirPressed(DIR_LEFT))
			{
				switch(GetCursorSelection())
				{
				case 0:
			...
			else if(pDI->KeyPressed(DIK_D) || pDI->KeyPressed(DIK_RIGHTARROW)  || pDI->JoystickDPadPressed(DIR_RIGHT) || pDI->JoystickGetLStickDirPressed(DIR_RIGHT))
			{
				switch(GetCursorSelection())
				{
				case 0:
